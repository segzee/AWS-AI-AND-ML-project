...
def get_data_loaders(
    ...
    data_transforms = {
        "train": T.Compose([
        T.Resize(256),
        T.RandomAffine(scale=(0.9, 1.1), degrees=10),
...
def visualize_batch_grid(data_loaders, num_rows: int = 1, num_cols: int = 5):
    """
    ...
    base_path = Path(get_data_location())
    dataset = datasets.ImageFolder(base_path / "train")
    ...
    total_images = min(num_rows * num_cols, len(images))
    ...
    for i in range(total_images):
        if num_rows > 1:
            ...
            ax = axes[i]  # For a single row, use a 1D array of axes
        ax.imshow(images[i])
...
def test_visualize_batch_grid(data_loaders):

    visualize_batch_grid(data_loaders, num_rows = 1, num_cols = 5)